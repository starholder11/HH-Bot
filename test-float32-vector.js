...
async function testFloat32Vector() {
  try {
    console.log('🚀 Testing LanceDB with Float32Array vectors...');
    ...
    // Create 300 rows of data with Float32Array
    const data = [];
    for (let i = 0; i < 300; i++) {
      // Create slight variations of the base embedding as Float32Array
      const embedding = new Float32Array(baseEmbedding.map(val => val + (Math.random() - 0.5) * 0.01));

      data.push({
        id: `test-row-${i}`,
        content_type: 'text',
        title: `Test Row ${i}`,
        embedding: embedding, // Use Float32Array
    ...
    const searchEmbedding = new Float32Array(baseEmbedding);
