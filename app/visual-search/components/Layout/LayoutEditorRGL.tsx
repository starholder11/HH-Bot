...
export default function LayoutEditorRGL({ layout, onClose, onSaved }: Props) {
  ...
  // Convert items to responsive layout format for RGL
  const layouts = useMemo(() => {
    const gridLayout = edited.layout_data.items.map(item => ({
      i: item.id,
      x: item.x,
      y: item.y,
      w: item.w,
  ...
  const duplicateItem = useCallback((id: string) => {
    ...
    // Simple keyboard shortcuts for selected item
  React.useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (e.key === 'ArrowRight') { e.preventDefault(); nudgeItem(step, 0); }
      if (e.key === 'ArrowUp') { e.preventDefault(); nudgeItem(0, -step); }
      if (e.key === 'ArrowDown') { e.preventDefault(); nudgeItem(0, step); }
